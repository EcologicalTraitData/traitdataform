...
2. mapping of trait concepts onto a user-provided traitlist (i.e. a thesaurus of traits) or globally accessible URIs  (see section [2. Standardize traits](#standardize-traits)) and unify units and factor levels (see vignete ['Handling units with trait-data'](traitdataform-units.html)),
...
The detailled metadata information of both datasets (e.g. license and bibliographic citation) will be stored in the attributes of the dataset and displayed when calling it in R console. You can access the metadata via the `attributes()`  function. E.g.
...
Along with these primary data, you should make any ancilliary data table available along with the data, e.g. the metadata in a human readable form, as well as the lookup table of traits and taxa:
